...
redisDF = redisDF.selectExpr("cast(value as string) value")
...
redisDF.withColumn("value", from_json("value", redis_schema)) \
...
encodedCustomer = encodedCustomer.withColumn("customer", unbase64(encodedCustomer.encodedCustomer).cast("string"))
...
encodedCustomer.withColumn("customer", from_json("customer", customer_record_schema)) \
    .select(col("customer.*")) \
...
# TO-DO: cast the value column in the streaming dataframe as a STRING
stediDF = stediDF.selectExpr("cast(value as string) value")
...
stediDF.withColumn("value", from_json("value", stedi_schema)) \
